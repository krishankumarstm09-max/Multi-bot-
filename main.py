...
def connect_bot_save(message):
    ...
    try:
        row=mdb("""INSERT INTO managed_bots(owner_id,owner_user_id,bot_token,bot_id,bot_username,bot_name,status)
                   VALUES(%s,%s,%s,%s,%s,%s,'active') RETURNING id""",(message.from_user.id,message.from_user.id,token,me.id,me.username or '',me.first_name or ''),one=True,commit=True)
        bot_db_id=row[0]
        ensure_managed_defaults(bot_db_id)
