...
if not key:
    ...
    log.warning("TOKEN_ENCRYPTION_KEY was not supplied. Generated temporary key; set a persistent key before production.")
...
def cb_bots(c):
    rows=db("SELECT id,bot_username,bot_name,active FROM managed_bots WHERE owner_id=%s ORDER BY id DESC",(c.from_user.id,),True)
    k=types.InlineKeyboardMarkup()
    for r in rows:
        k.add(types.InlineKeyboardButton(
          f"{'🟢' if r['active'] else '🔴'} @{r['bot_username'] or r['bot_name'] or r['id']}",
...
def cb_bot(c):
    ...
    r=db("SELECT bot_username,bot_name FROM managed_bots WHERE id=%s AND owner_id=%s",(pk,c.from_user.id),True)
    if not r:return
...
def cb_module(c):
    ...
    if not owner_of(pk,c.from_user.id): return
    if mod=="tutorials":
...
def module_botplans(c,pk):
    ...
    text="💎 <b>Membership</b>\n\n"+("No plans." if not rows else "\n".join(f"{'🟢' if x['enabled'] else '🔴'} <b>{x['name']}</b> • ₹{x['price']} • {x['duration_days']}d\n{x['description'][:80]}" for x in rows))
    k=types.InlineKeyboardMarkup(); k.add(types.InlineKeyboardButton("➕ Create Plan",callback_data=f"act:createplan:{pk}")); k.add(types.InlineKeyboardButton("👤 Manage User",callback_data=f"act:memberuser:{pk}")); k.add(types.InlineKeyboardButton("📋 Active Members",callback_data=f"act:members:{pk}")); k.add(types.InlineKeyboardButton("📚 Tutorial",callback_data=f"tt:membership:{pk}")); k.add(types.InlineKeyboardButton("⬅️ Back",callback_data=f"bot:{pk}"))
    manager.edit_message_text(text,c.message.chat.id,c.message.id,reply_markup=k)
...
def send_payload(inst,user_id,text,media_type=None,media_id=None,pk=None,source_user=None,channel_name=""):
    ...
    if media_type=="photo" and media_id:
        return inst.send_photo(user_id,media_id,caption=text or None)
    if media_type=="video" and media_id:
        return inst.send_video(user_id,media_id,caption=text or None)
    if media_type=="document" and media_id:
        return inst.send_document(user_id,media_id,caption=text or None)
    if media_type=="audio" and media_id:
        return inst.send_audio(user_id,media_id,caption=text or None)
    return inst.send_message(user_id,text or " ")
