...
def init_db():
    ...
    CREATE INDEX IF NOT EXISTS idx_event_logs_bot ON bot_event_logs(bot_id,created_at);
    ...
    command_defaults = [
      ("start", "Open welcome/start flow"),
      ("help", "Show available bot commands"),
      ("id", "Show your Telegram user ID"),
      ("status", "Show bot and membership status"),
    ]
    # Existing command rows are preserved; missing defaults are added only.
    for bot in db("SELECT id FROM managed_bots", fetch=True):
        for cmd, desc in command_defaults:
            db("INSERT INTO bot_commands(bot_id,command,description) VALUES(%s,%s,%s) ON CONFLICT(bot_id,command) DO NOTHING", (bot['id'],cmd,desc))

    tutorials = {
      "dashboard": ("🏠 Dashboard", "Open with <code>/start</code>. From here use My Bots, Connect Bot, Tutorial Center, Membership Plans, Remove Watermark and Manager Settings."),
      "start": ("🏠 Start Message", "Set the managed bot welcome message.\n\n<b>Variables:</b> <code>{first_name}</code> <code>{last_name}</code> <code>{username}</code> <code>{user_id}</code>\n\n<b>Flow:</b> Open Start Message → Edit Text → optionally Set Media → Buttons → Preview."),
      "buttons": ("🔘 Buttons", "Add inline buttons.\n\n<b>Command:</b> <code>Label | https://example.com</code>\n<b>Callback:</b> <code>Label | callback:action_name</code>\n\nChoose Start, Auto-DM or Broadcast scope, then Add Button."),
      "force_join": ("📢 Force Join", "Add required channels before users can continue.\n\n<b>Exact command:</b> <code>#g Channel_Name -1001234567890 https://t.me/example</code>\n\n<code>#r</code> = red label, <code>#g</code> = green, <code>#b</code> = blue. The bot must have permission to check the channel. Pending Join Requests are tracked without automatic approval."),
      "broadcast": ("📣 Broadcast", "Create a broadcast from text/photo/video/document. The manager sends it to active users and records sent/failed delivery logs. Use the New Broadcast button and follow the displayed steps."),
      "autodm": ("📩 Auto-DM", "Configure automatic private messages for <code>join_request</code> and <code>new_member</code>. Supports text, media, delay and variables: <code>{first_name}</code>, <code>{last_name}</code>, <code>{username}</code>, <code>{user_id}</code>, <code>{channel_name}</code>."),
      "support": ("💬 Support Inbox", "User messages received by a managed bot are routed to the owner. Reply directly to the mapped manager message to send the response back to the user."),
      "users": ("👥 Users", "Search by Telegram numeric ID or @username. Open a profile to see status, join/last-seen information, premium status and notes; then block, unblock, manage premium or add a note."),
      "channels": ("📢 Channels", "Manage configured channels: add, enable/disable and delete. The same channel records power the Force Join system."),
      "membership": ("💎 Membership", "Create plans using <code>Name | Price | Days | Description</code>. Example: <code>Pro | 249 | 90 | Remove watermark</code>. Then search a user and Grant, Extend or Revoke membership. Expiry is tracked automatically."),
      "watermark": ("💧 Watermark", "Enable/disable the watermark, edit its text and preview it. Active premium members automatically receive messages without the watermark."),
      "statistics": ("📊 Statistics", "View users, active/blocked users, channels, broadcasts, membership and other bot-level counters from the Statistics module."),
      "tools": ("🧰 Admin Tools", "Diagnostics and operations: Health Check verifies Telegram API and PostgreSQL, Live Stats shows current counters, Export Users creates a CSV, Commands documents bot commands, and Audit Log shows recent administrator actions."),
      "settings": ("⚙️ Settings", "Configure manager-facing bot settings such as support username, news channel, privacy policy and language from the Settings module."),
      "tutorials": ("📚 Tutorial Center", "Every major module has a dedicated tutorial with purpose, syntax, variables and step-by-step flow."),
    }
    for fk,(title,body) in tutorials.items():
        db("INSERT INTO tutorials(feature_key,title,body) VALUES(%s,%s,%s) ON CONFLICT(feature_key) DO UPDATE SET title=EXCLUDED.title,body=EXCLUDED.body,updated_at=CURRENT_TIMESTAMP",(fk,title,body))
...
def ensure_default_commands(pk):
    defaults=[('start','Open welcome/start flow'),('help','Show available bot commands'),('id','Show your Telegram user ID'),('status','Show bot and membership status')]
    for cmd,desc in defaults:
        db("INSERT INTO bot_commands(bot_id,command,description) VALUES(%s,%s,%s) ON CONFLICT(bot_id,command) DO NOTHING",(pk,cmd,desc))

def log_event(pk,event_type,user_id=None,details=''):
    try:
        db("INSERT INTO bot_event_logs(bot_id,user_id,event_type,details) VALUES(%s,%s,%s,%s)",(pk,user_id,event_type,details[:500]))
    except Exception:
        pass
...
def home_kb(uid):
    ...
    k.add(
        types.InlineKeyboardButton("🤖 My Bots", callback_data="mbots"),
        types.InlineKeyboardButton("➕ Connect Bot", callback_data="connect"),
    )
    k.add(
        types.InlineKeyboardButton("📚 Tutorial Center", callback_data="tutorials"),
        types.InlineKeyboardButton("💎 Membership Plans", callback_data="plans"),
    )
    k.add(
        types.InlineKeyboardButton("💧 Remove Watermark", callback_data="remove_watermark"),
        types.InlineKeyboardButton("⚙️ Manager Settings", callback_data="msettings"),
    )
    if is_master(uid):
        k.add(types.InlineKeyboardButton("👑 Master Admin", callback_data="master"))
    return k
...
# ---------------- MANAGER BOT ----------------
@manager.message_handler(commands=["help", "commands"] )
def help_command(m):
    save_manager_user(m.from_user)
    text=(
        f"🤖 <b>{MANAGER_NAME} Command Center</b>\n\n"
        "<b>Manager commands</b>\n"
        "<code>/start</code> — Open main dashboard\n"
        "<code>/admin</code> — Open Master Admin (owner only)\n"
        "<code>/bots</code> — Open My Bots\n"
        "<code>/tutorials</code> — Open Tutorial Center\n"
        "<code>/plans</code> — View Membership Plans\n"
        "<code>/connect</code> — Connect a managed bot\n"
        "<code>/cancel</code> — Cancel the current setup\n\n"
        "Open <b>My Bots</b> to access every module. Each module contains its command syntax, description and tutorial."
    )
    manager.send_message(m.chat.id,text,reply_markup=home_kb(m.from_user.id))

@manager.message_handler(commands=["bots"])
def bots_command(m):
    save_manager_user(m.from_user)
    rows=db("SELECT id,bot_username,bot_name,active FROM managed_bots WHERE owner_id=%s ORDER BY id DESC",(m.from_user.id,),True)
    k=types.InlineKeyboardMarkup()
    for r in rows:
        k.add(types.InlineKeyboardButton(f"{'🟢' if r['active'] else '🔴'} @{r['bot_username'] or r['bot_name'] or r['id']}",callback_data=f"bot:{r['id']}"))
    k.add(types.InlineKeyboardButton("➕ Connect Bot",callback_data="connect"),types.InlineKeyboardButton("⬅️ Home",callback_data="home"))
    manager.send_message(m.chat.id,"🤖 <b>My Bots</b>\n\nSelect a connected bot or connect a new one.",reply_markup=k)

@manager.message_handler(commands=["tutorials"])
def tutorials_command(m):
    save_manager_user(m.from_user)
    rows=db("SELECT feature_key,title FROM tutorials ORDER BY id",fetch=True)
    k=types.InlineKeyboardMarkup(row_width=2)
    for r in rows:k.add(types.InlineKeyboardButton(r["title"],callback_data=f"tt:{r['feature_key']}:0"))
    k.add(types.InlineKeyboardButton("⬅️ Home",callback_data="home"))
    manager.send_message(m.chat.id,"📚 <b>Tutorial Center</b>\n\nEvery feature has its command/syntax and step-by-step setup here.",reply_markup=k)

@manager.message_handler(commands=["connect"])
def connect_command(m):
    STATE[m.from_user.id]={"action":"connect_token"}
    manager.send_message(m.chat.id,"➕ <b>Connect Bot</b>\n\nSend the BotFather token.\n\n<code>/cancel</code> — cancel",reply_markup=back_home())
...
def receive_token(m):
    ...
    try:
        ...
        ensure_default_commands(bot_pk)
...
def bot_panel(bot_pk):
    k=types.InlineKeyboardMarkup(row_width=2)
    rows=[
      ("🏠 Start Message","startset"),("🔘 Buttons","buttons"),
      ("📢 Force Join","fj"),("📣 Broadcast","broadcast"),
      ("📩 Auto-DM","autodm"),("💬 Support Inbox","support"),
      ("👥 Users","users"),("📢 Channels","channels"),
      ("💎 Membership","botplans"),("💧 Watermark","watermark"),
      ("📊 Statistics","stats"),("🧰 Admin Tools","tools"),
      ("⚙️ Settings","botsettings"),("📚 Tutorials","tutorials"),
      ("⏯ Bot Status","botstatus"),
    ]
    for i in range(0,len(rows),2):
        pair=rows[i:i+2]
        k.row(*[types.InlineKeyboardButton(label,callback_data=f"m:{code}:{bot_pk}") for label,code in pair])
    k.row(types.InlineKeyboardButton("🗑 Remove Bot",callback_data=f"m:deletebot:{bot_pk}"))
    k.row(types.InlineKeyboardButton("⬅️ My Bots",callback_data="mbots"))
    return k
...
BUTTON_INFO = (
    "🔘 <b>Buttons Builder</b>\n\n"
    "Create inline buttons for Start, Auto-DM and Broadcast messages.\n\n"
    "<b>Command format</b>\n"
    "<code>Button Label | https://example.com</code>\n"
    "or\n"
    "<code>Button Label | callback:my_action</code>\n\n"
    "<b>Notes</b>\n"
    "• URL buttons open the supplied HTTPS/HTTP link.\n"
    "• Callback buttons are stored for your bot workflow.\n"
    "• Use the Add Button button below to save a button."
)

def module_buttons(c,pk):
    if not owner_guard(pk,c.from_user.id): return
    rows=db("SELECT scope,label,url,callback_data,row_no FROM bot_buttons WHERE bot_id=%s ORDER BY scope,row_no,id",(pk,),True)
    lines=[]
    for r in rows:
        target=r['url'] or ('callback:'+r['callback_data'] if r['callback_data'] else '-')
        lines.append(f"• <b>{r['scope'].title()}</b> — {r['label']} → <code>{target}</code>")
    text=BUTTON_INFO+"\n\n"+(("<b>Saved buttons</b>\n"+"\n".join(lines)) if lines else "<i>No buttons saved yet.</i>")
    k=types.InlineKeyboardMarkup(row_width=2)
    k.row(types.InlineKeyboardButton("➕ Start Button",callback_data=f"buttons:start:{pk}"),types.InlineKeyboardButton("➕ Auto-DM Button",callback_data=f"buttons:autodm:{pk}"))
    k.row(types.InlineKeyboardButton("➕ Broadcast Button",callback_data=f"buttons:broadcast:{pk}"))
    k.add(types.InlineKeyboardButton("📚 Tutorial",callback_data=f"tt:buttons:{pk}"))
...
@manager.callback_query_handler(func=lambda c:c.data=="remove_watermark")
def remove_watermark_root(c):
    expire_memberships()
    rows=db("SELECT id,name,price,duration_days,description FROM membership_plans WHERE enabled=TRUE ORDER BY price",fetch=True)
    text=("💧 <b>Remove Watermark</b>\n\n"
          "Choose an admin-created membership plan. After selecting a plan, contact the configured admin to activate it manually.\n\n"
          + ("\n".join(f"💎 <b>{r['name']}</b> — ₹{r['price']} / {r['duration_days']} days\n{r['description'] or ''}" for r in rows) if rows else "No active plans are available yet."))
    k=types.InlineKeyboardMarkup()
    for r in rows:k.add(types.InlineKeyboardButton(f"💎 {r['name']} — ₹{r['price']}",callback_data=f"pickplan:{r['id']}"))
    k.add(types.InlineKeyboardButton("⬅️ Home",callback_data="home"))
    safe_edit(c,text,k)
...
# ---------------- ADMIN TOOLS ----------------
def module_tools(c,pk):
    if not owner_guard(pk,c.from_user.id): return
    r=db("SELECT bot_username,bot_name,bot_id,active,created_at,updated_at FROM managed_bots WHERE id=%s",(pk,),True)
    if not r:return
    users=db("SELECT COUNT(*) n FROM bot_users WHERE bot_id=%s",(pk,),True)['n']
    events=db("SELECT COUNT(*) n FROM bot_event_logs WHERE bot_id=%s",(pk,),True)['n']
    text=(f"🧰 <b>Admin Tools</b>\n\n"
          f"🤖 @{r['bot_username'] or r['bot_name']}\n"
          f"🆔 <code>{r['bot_id']}</code>\n"
          f"Status: {'🟢 Online' if r['active'] else '🔴 Paused'}\n"
          f"👥 Users: <b>{users}</b>\n"
          f"🧾 Events logged: <b>{events}</b>\n\n"
          "Use the tools below for diagnostics, exports and command documentation.")
    k=types.InlineKeyboardMarkup(row_width=2)
    k.row(types.InlineKeyboardButton("🩺 Health Check",callback_data=f"tool:health:{pk}"),types.InlineKeyboardButton("📊 Live Stats",callback_data=f"tool:livestats:{pk}"))
    k.row(types.InlineKeyboardButton("📤 Export Users",callback_data=f"tool:export:{pk}"),types.InlineKeyboardButton("⌨️ Commands",callback_data=f"tool:commands:{pk}"))
    k.row(types.InlineKeyboardButton("🧾 Audit Log",callback_data=f"tool:audit:{pk}"),types.InlineKeyboardButton("📚 Tutorial",callback_data=f"tt:tools:{pk}"))
    k.add(types.InlineKeyboardButton("⬅️ Back",callback_data=f"bot:{pk}"))
    safe_edit(c,text,k)

@manager.callback_query_handler(func=lambda c:c.data.startswith("tool:health:"))
def tool_health(c):
    pk=int(c.data.split(":")[2])
    if not owner_guard(pk,c.from_user.id):return
    r=bot_row(pk)
    checks=[]
    try:
        token=decrypt_token(r['token_ciphertext']); inst=MANAGED.get(pk)
        if inst is None: inst=telebot.TeleBot(token,parse_mode='HTML')
        me=inst.get_me(); checks.append(f"Telegram API: 🟢 @{me.username or me.id}")
    except Exception as e: checks.append(f"Telegram API: 🔴 {str(e)[:120]}")
    try:
        db("SELECT 1",fetch=True); checks.append("PostgreSQL: 🟢 Connected")
    except Exception as e: checks.append(f"PostgreSQL: 🔴 {str(e)[:120]}")
    k=types.InlineKeyboardMarkup();k.add(types.InlineKeyboardButton("⬅️ Admin Tools",callback_data=f"m:tools:{pk}"))
    safe_edit(c,"🩺 <b>Health Check</b>\n\n"+"\n".join(checks),k)

@manager.callback_query_handler(func=lambda c:c.data.startswith("tool:livestats:"))
def tool_live_stats(c):
    pk=int(c.data.split(":")[2])
    if not owner_guard(pk,c.from_user.id):return
    vals=db("""SELECT
      (SELECT COUNT(*) FROM bot_users WHERE bot_id=%s) users,
      (SELECT COUNT(*) FROM bot_users WHERE bot_id=%s AND status='active') active,
      (SELECT COUNT(*) FROM bot_users WHERE bot_id=%s AND last_seen>CURRENT_TIMESTAMP-INTERVAL '24 hours') seen24,
      (SELECT COUNT(*) FROM memberships WHERE bot_id=%s AND status='active' AND expires_at>CURRENT_TIMESTAMP) premium,
      (SELECT COUNT(*) FROM broadcast_logs bl JOIN broadcasts b ON b.id=bl.broadcast_id WHERE b.bot_id=%s AND bl.status='sent') delivered
    """,(pk,pk,pk,pk,pk),True)
    k=types.InlineKeyboardMarkup();k.add(types.InlineKeyboardButton("🔄 Refresh",callback_data=f"tool:livestats:{pk}"),types.InlineKeyboardButton("⬅️ Tools",callback_data=f"m:tools:{pk}"))
    safe_edit(c,f"📊 <b>Live Statistics</b>\n\n👥 Total users: <b>{vals['users']}</b>\n🟢 Active: <b>{vals['active']}</b>\n👀 Seen 24h: <b>{vals['seen24']}</b>\n💎 Premium: <b>{vals['premium']}</b>\n📨 Broadcast delivered: <b>{vals['delivered']}</b>",k)

@manager.callback_query_handler(func=lambda c:c.data.startswith("tool:commands:"))
def tool_commands(c):
    pk=int(c.data.split(":")[2])
    if not owner_guard(pk,c.from_user.id):return
    ensure_default_commands(pk)
    rows=db("SELECT command,description,enabled FROM bot_commands WHERE bot_id=%s ORDER BY command",(pk,),True)
    text="⌨️ <b>Bot Commands</b>\n\n"+"\n".join(f"• <code>/{x['command']}</code> — {x['description']} {'🟢' if x['enabled'] else '🔴'}" for x in rows)
    text+="\n\n<b>Module commands</b> are configured from the manager dashboard.\n/cancel is available in every manager setup flow."
    k=types.InlineKeyboardMarkup();k.add(types.InlineKeyboardButton("⬅️ Tools",callback_data=f"m:tools:{pk}"))
    safe_edit(c,text,k)

@manager.callback_query_handler(func=lambda c:c.data.startswith("tool:audit:"))
def tool_audit(c):
    pk=int(c.data.split(":")[2])
    if not owner_guard(pk,c.from_user.id):return
    rows=db("SELECT actor_id,action,target_user_id,details,created_at FROM audit_logs WHERE bot_id=%s ORDER BY id DESC LIMIT 15",(pk,),True)
    lines=[]
    for x in rows:
        lines.append(f"• <b>{x['action']}</b> — actor <code>{x['actor_id']}</code>"+(f" → <code>{x['target_user_id']}</code>" if x['target_user_id'] else ""))
    k=types.InlineKeyboardMarkup();k.add(types.InlineKeyboardButton("⬅️ Tools",callback_data=f"m:tools:{pk}"))
    safe_edit(c,"🧾 <b>Recent Audit Log</b>\n\n"+("\n".join(lines) if lines else "No audit events yet."),k)

@manager.callback_query_handler(func=lambda c:c.data.startswith("tool:export:"))
def tool_export(c):
    pk=int(c.data.split(":")[2])
    if not owner_guard(pk,c.from_user.id):return
    rows=db("SELECT user_id,username,first_name,last_name,status,joined_at,last_seen FROM bot_users WHERE bot_id=%s ORDER BY id",(pk,),True)
    import csv, tempfile
    fd,path=tempfile.mkstemp(prefix=f"bot_{pk}_users_",suffix=".csv")
    os.close(fd)
    with open(path,'w',newline='',encoding='utf-8') as f:
        w=csv.writer(f);w.writerow(['user_id','username','first_name','last_name','status','joined_at','last_seen'])
        for x in rows:w.writerow([x['user_id'],x['username'] or '',x['first_name'] or '',x['last_name'] or '',x['status'],x['joined_at'],x['last_seen']])
    try:
        with open(path,'rb') as f: manager.send_document(c.message.chat.id,f,caption=f"📤 User export — {len(rows)} records")
    finally:
        try:os.remove(path)
        except:pass
    manager.answer_callback_query(c.id,"Export ready ✅")
...
def install_handlers(inst,pk):
    ...
    def managed_start(m):
        ...
        log_event(pk,"start",m.from_user.id)
    ...
    ensure_default_commands(pk)
    try:
        inst.set_my_commands([types.BotCommand('start','Open welcome/start flow'),types.BotCommand('help','Show available bot commands'),types.BotCommand('id','Show your Telegram user ID'),types.BotCommand('status','Show bot and membership status')])
    except Exception:
        pass

    @inst.message_handler(commands=['help'])
    def managed_help(m):
        save_bot_user(pk,m.from_user)
        rows=db("SELECT command,description FROM bot_commands WHERE bot_id=%s AND enabled=TRUE ORDER BY command",(pk,),True)
        text="🤖 <b>Bot Help</b>\n\n"+"\n".join(f"/{x['command']} — {x['description']}" for x in rows)
        inst.send_message(m.chat.id,text)

    @inst.message_handler(commands=['id'])
    def managed_id(m):
        save_bot_user(pk,m.from_user)
        inst.send_message(m.chat.id,f"🆔 Your Telegram ID: <code>{m.from_user.id}</code>")

    @inst.message_handler(commands=['status'])
    def managed_status(m):
        save_bot_user(pk,m.from_user)
        premium='💎 Premium active' if member_active(pk,m.from_user.id) else '👤 Standard account'
        inst.send_message(m.chat.id,f"📊 <b>Account Status</b>\n\n{premium}\n🆔 <code>{m.from_user.id}</code>")

    @inst.message_handler(content_types=["text","photo","video","document","audio","voice","animation"])
    def managed_message(m):
        if m.text and m.text.startswith("/"):return
        save_bot_user(pk,m.from_user)
        log_event(pk,"message",m.from_user.id,m.content_type)
    ...
    def join_request(req):
        ...
        cid=str(req.chat.id)
        log_event(pk,"join_request",req.from_user.id,str(req.chat.id))
    ...
    def member_update(update):
        try:
            ...
            if new in ("member","administrator","creator") and old not in ("member","administrator","creator"):
                ...
                log_event(pk,"new_member",update.new_chat_member.user.id,str(update.chat.id))
