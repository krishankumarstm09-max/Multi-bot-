...
def make_url_button(title, url):
    ...
    try:
        ...
        # Older pyTelegramBotAPI versions do not know Telegram's newer
        # button-style field.  Keep the button fully functional without it.
...
def send_force_join_page(chat_id, user):
    ...
    markup.add(types.InlineKeyboardButton(text=verify_title,callback_data='verify_force_join',style=verify_style or 'primary'))
    if m_type=='photo' and m_id!='NONE':
        _send_media_formatted(bot,'photo',chat_id,m_id,txt,entities,markup)
    elif m_type=='video' and m_id!='NONE':
        _send_media_formatted(bot,'video',chat_id,m_id,txt,entities,markup)
    else:
        try: bot.send_message(chat_id,txt,entities=entities or None,reply_markup=markup)
        except Exception: bot.send_message(chat_id,txt,parse_mode='HTML',reply_markup=markup)
...
def send_start_page(chat_id, user):
    ...
    if m_type=='photo' and m_id!='NONE':
        return _send_media_formatted(bot,'photo',chat_id,m_id,txt,entities,markup)
    if m_type=='video' and m_id!='NONE':
        return _send_media_formatted(bot,'video',chat_id,m_id,txt,entities,markup)
    try:
        return bot.send_message(chat_id,txt,entities=entities or None,reply_markup=markup)
    except Exception:
        return bot.send_message(chat_id,txt,parse_mode='HTML',reply_markup=markup)
    ...
    try:
        ...
        for d in data or []:
            ...
            try:
                ...
                # Older pyTelegramBotAPI versions may not accept newer fields.
                kw.pop('custom_emoji_id',None)
                try: result.append(types.MessageEntity(**kw))
...
def _send_formatted(bot_obj, chat_id, text, entities=None, reply_markup=None, **kwargs):
    """Send using entities first; fall back to HTML/plain for old libraries."""
    entities=entities or []
    try:
        return bot_obj.send_message(chat_id,text,entities=entities or None,reply_markup=reply_markup,**kwargs)
    except Exception as first_error:
        try:
            return bot_obj.send_message(chat_id,text,parse_mode='HTML',reply_markup=reply_markup,**kwargs)
        except Exception:
            if kwargs.get('caption') is not None:
                raise first_error
            raise

def _send_media_formatted(bot_obj, kind, chat_id, media_id, caption, entities=None, reply_markup=None, **kwargs):
    entities=entities or []
    method=bot_obj.send_photo if kind=='photo' else bot_obj.send_video
    entity_kw='caption_entities'
    try:
        return method(chat_id,media_id,caption=caption,**{entity_kw:(entities or None)},reply_markup=reply_markup,**kwargs)
    except Exception as first_error:
        try:
            return method(chat_id,media_id,caption=caption,parse_mode='HTML',reply_markup=reply_markup,**kwargs)
        except Exception:
            raise first_error
...
def managed_send_payload(mbot, chat_id, bot_db_id, prefix, pin=False, user=None):
    ...
    txt,entities=_get_formatted_payload(mget,prefix+'_text',user)
    ...
    if mt=='photo' and mid!='NONE':
        msg=_send_media_formatted(mbot,'photo',chat_id,mid,txt,entities,markup)
    elif mt=='video' and mid!='NONE':
        msg=_send_media_formatted(mbot,'video',chat_id,mid,txt,entities,markup)
    else:
        try:
            msg=mbot.send_message(chat_id,txt,entities=entities or None,reply_markup=markup)
        except Exception:
            # Legacy HTML fallback for settings created by older versions.
            msg=mbot.send_message(chat_id,txt,parse_mode='HTML',reply_markup=markup)
    if pin and msg:
...
def managed_start_page(mbot,bot_db_id,user):
    ...
    if missing:
        ...
        txt,entities=_get_formatted_payload(mget,'fj_text',user)
        mid=mget(bot_db_id,'fj_media_id'); mt=mget(bot_db_id,'fj_media_type')
        if mt=='photo' and mid!='NONE':
            _send_media_formatted(mbot,'photo',user.id,mid,txt,entities,mk)
        elif mt=='video' and mid!='NONE':
            _send_media_formatted(mbot,'video',user.id,mid,txt,entities,mk)
        else:
            try: mbot.send_message(user.id,txt,entities=entities or None,reply_markup=mk)
            except Exception: mbot.send_message(user.id,txt,parse_mode='HTML',reply_markup=mk)
        return
...
def managed_register_handlers(mbot,bot_db_id):
    ...
    # pyTelegramBotAPI versions differ. Register chat_member only when the
    # installed library supports it; the rest of the connected bot must keep
    # working even on an older deployment.
    if hasattr(mbot, 'chat_member_handler'):
        mbot.chat_member_handler()(_member_update)
...
def ensure_managed_runtime(bot_db_id, wait_seconds=2.0):
    """Ensure a saved connected bot is actually running in this process.

    Railway restarts clear MANAGED_BOTS, while the PostgreSQL row remains.
    This lazy-start path makes Preview/Start/Force Join/etc. recover without
    requiring the user to reconnect the token.
    """
    mbot=MANAGED_BOTS.get(bot_db_id)
    if mbot is not None:
        return mbot
    ...
    try:
        ...
        print('Lazy managed bot start failed:',e)
    return MANAGED_BOTS.get(bot_db_id)

def managed_preview(chat_id,bot_db_id,prefix):
    mbot=ensure_managed_runtime(bot_db_id)
    if not mbot:
        bot.send_message(chat_id,'❌ Connected bot is not running. Use ♻️ Reconnect Bot once.')
        return
    try:
        # Preview uses the connected bot itself, so Telegram renders the exact
        # saved entities/custom emoji/media/buttons.
        user=managed_user_for_chat(bot_db_id,chat_id)
        if user is None:
            class U: pass
            user=U(); user.id=chat_id; user.first_name='Preview'; user.username=''
        managed_send_payload(mbot,chat_id,bot_db_id,prefix,False,user)
    except Exception as e:
        print('Managed preview error:',repr(e))
        bot.send_message(chat_id,'❌ Preview failed: '+str(e)[:700])
...
def managed_send_reply_text_connected(message,bot_db_id,user_id):
    """Send an owner reply through the connected bot with original Telegram entities."""
    mbot=MANAGED_BOTS.get(bot_db_id)
    ...
    try:
        ...
        try:
            mbot.send_message(user_id,message.text,entities=entities or None)
        except Exception:
            mbot.send_message(user_id,_message_html(message),parse_mode='HTML')
...
def send_broadcast_preview(chat_id):
    ...
    try:
        if m_type=='photo' and m_id!='NONE': return _send_media_formatted(bot,'photo',chat_id,m_id,txt,entities,markup)
        if m_type=='video' and m_id!='NONE': return _send_media_formatted(bot,'video',chat_id,m_id,txt,entities,markup)
        return bot.send_message(chat_id,txt,entities=entities or None,reply_markup=markup)
    except Exception as e:
        bot.send_message(chat_id,'❌ Preview error: '+str(e)[:500])
...
def execute_broadcast(admin_chat_id):
    ...
    for u in users:
        try:
            ...
            if m_type == "photo" and m_id != "NONE":
                m = _send_media_formatted(bot,'photo',u,m_id,txt,entities,markup)
            elif m_type == "video" and m_id != "NONE":
                m = _send_media_formatted(bot,'video',u,m_id,txt,entities,markup)
            else:
                try:
                    m = bot.send_message(u,txt,entities=entities or None,reply_markup=markup)
                except Exception:
                    m = bot.send_message(u,txt,parse_mode='HTML',reply_markup=markup)
