...
def send_force_join_page(chat_id, user):
    ...
    if m_type=='photo' and m_id!='NONE':
        ...
        return _send_formatted(bot,chat_id,txt,entities,markup)
...
def _send_formatted(bot_obj, chat_id, text, entities=None, reply_markup=None, **kwargs):
    """Send with Telegram entities; only legacy non-custom content may use HTML fallback."""
    entities=entities or []
    has_custom_emoji=any(getattr(e,'type',None)=='custom_emoji' for e in entities)
    try:
        return bot_obj.send_message(chat_id,text,entities=entities or None,reply_markup=reply_markup,**kwargs)
    except Exception as first_error:
        # HTML cannot reproduce Telegram custom-emoji document entities.
        if has_custom_emoji:
            raise first_error
        try:
...
def managed_send_payload(mbot, chat_id, bot_db_id, prefix, pin=False, user=None):
    ...
    if mt=='photo' and mid!='NONE':
        ...
        msg=_send_formatted(mbot,chat_id,txt,entities,markup)
    if pin and msg:
...
def managed_start_page(mbot,bot_db_id,user):
    ...
    if missing:
        ...
        if mt=='photo' and mid!='NONE':
            ...
            _send_formatted(mbot,user.id,txt,entities,mk)
        return
...
def managed_send_reply_text_connected(message,bot_db_id,user_id):
    ...
    try:
        ...
        _send_formatted(mbot,user_id,message.text,entities)
...
def handle_callbacks(call):
    ...
    if call.data.startswith('msee_'):
        ...
        if kind=='media': val=mget(bid,prefix+'_media_type')
            ...
            val, _ = _get_formatted_payload(lambda key: mget(bid,key),prefix+'_text',managed_user_for_chat(bid,chat_id))
    ...
    for u in users:
        try:
            ...
            if m_type == "photo" and m_id != "NONE":
                ...
                m = _send_formatted(bot,u,txt,entities,markup)
