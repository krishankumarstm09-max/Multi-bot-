...
def init_db():
    with DB_LOCK:
        ...
        # LeaveDM channels: completely separate from the Force-Join channel list.
        c.execute("""
            CREATE TABLE IF NOT EXISTS managed_leave_channels (
                id SERIAL PRIMARY KEY,
                bot_id INTEGER NOT NULL REFERENCES managed_bots(id) ON DELETE CASCADE,
                channel_id TEXT NOT NULL,
                title TEXT,
        ...
        # Controller Force-Join "join request" memory (used by handle_join_request).
        c.execute("""
...
# ============================================================
# ============================================================
# BUTTON LABEL HELPERS  (colors + Premium/custom emoji icons)
# ============================================================
# Premium emoji inside a button label is stored as a marker:
#     {ce:<custom_emoji_id>:<fallback emoji>}
# The marker is created automatically when the owner sends a button text that
# contains Telegram Premium emoji.  At send time it becomes Telegram's native
# `icon_custom_emoji_id` (Bot API 9.4+).  If Telegram rejects the icon, the
# plain fallback emoji is shown instead, so buttons never disappear.
_CE_MARK = re.compile(r'\{ce:(\d+):([^{}]*)\}')

_STYLE_MAP = {
    "#r": "danger",
    "#g": "success",
    "#b": "primary",
    "#l": "primary",   # kept for compatibility
    "#p": "primary",   # kept for compatibility
}


class _Btn(types.InlineKeyboardButton):
    """InlineKeyboardButton that ALWAYS serialises `style` and
    `icon_custom_emoji_id`, whatever pyTelegramBotAPI version is installed."""

    def __init__(self, text, *args, icon_custom_emoji_id=None, style=None,
                 fallback_text=None, **kwargs):
        try:
            super().__init__(text, *args, style=style, **kwargs)
        except TypeError:
            super().__init__(text, *args, **kwargs)
        self._icon = icon_custom_emoji_id or None
        self._style = style or None
        self._fallback_text = fallback_text or text

    def to_dict(self):
        data = super().to_dict()
        if self._style and 'style' not in data:
            data['style'] = self._style
        if self._icon and 'icon_custom_emoji_id' not in data:
            data['icon_custom_emoji_id'] = self._icon
        return data

    def plain(self):
        """Same button without the Premium-emoji icon (used as fallback)."""
        twin = copy.copy(self)
        twin._icon = None
        twin.text = self._fallback_text
        return twin


def parse_button_label(raw):
    """Return (label, style, icon_custom_emoji_id, fallback_label)."""
    text = (raw or "").strip()
    style = None
    lower = text.lower()
    for code, st in _STYLE_MAP.items():
        if lower.startswith(code):
            text = text[len(code):].strip()
            style = st
            break

    fallback = _CE_MARK.sub(lambda m: m.group(2), text).strip()
    icon = None
    label = fallback
    first = _CE_MARK.search(text)
    if first:
        icon = first.group(1)
        before = text[:first.start()]
        after = _CE_MARK.sub(lambda m: m.group(2), text[first.end():])
        joiner = " " if before.strip() and after.strip() else ""
        label = (before.rstrip() + joiner + after.lstrip()).strip()
        if not label:            # icon-only label: keep the emoji character
            icon = None
            label = fallback
    return label, style, icon, fallback
...
def get_button_style_and_text(text):
    ...
    #r = red (danger)   #g = green (success)   #b = blue (primary)
    #l and #p are kept for compatibility and map to blue.

    Backward compatible: returns (clean_text, style). Premium-emoji markers
    are turned back into their normal emoji characters here.
    """
    if not text:
        return "", None
    _, style, _, fallback = parse_button_label(text)
...
def _new_markup(row_width=2):
    """InlineKeyboardMarkup that also keeps an icon-free twin for fallback."""
    markup = types.InlineKeyboardMarkup(row_width=row_width)
    markup._twin = types.InlineKeyboardMarkup(row_width=row_width)
    markup._has_icons = False
    return markup


def _markup_add(markup, *buttons):
    markup.add(*buttons)
    twin = getattr(markup, "_twin", None)
    if twin is not None:
        twin.add(*[b.plain() if hasattr(b, "plain") else b for b in buttons])
    if any(getattr(b, "_icon", None) for b in buttons):
        markup._has_icons = True
    return markup


def make_url_button(title, url):
    """Create a native Telegram styled inline URL button (Premium icon aware)."""
    label, style, icon, fallback = parse_button_label(title)
    return _Btn(label or fallback or "•", url=url.strip(), style=style,
                icon_custom_emoji_id=icon, fallback_text=fallback)


def make_callback_button(title, callback_data, default_style=None, empty_text=None):
    """Create a callback button; #r/#g/#b controls color, Premium emoji -> icon."""
    label, style, icon, fallback = parse_button_label(title)
    return _Btn(label or fallback or empty_text or "•", callback_data=callback_data,
                style=style or default_style, icon_custom_emoji_id=icon,
                fallback_text=fallback or empty_text)
...
def build_keyboard_from_string(btn_str):
    ...
    A Premium emoji in the label becomes a real Telegram button icon.
    """
    markup = _new_markup(row_width=2)
    ...
    for line in btn_str.strip().splitlines():
        ...
        if title and url:
            _markup_add(markup, make_url_button(title, url))

    return markup


def encode_custom_emoji_markers(text, entities):
    """Replace every Premium-emoji span of a message with {ce:ID:char}.

    Telegram entity offsets are UTF-16 based, so slicing is done on UTF-16.
    """
    text = text or ""
    ces = [e for e in (entities or [])
    ...
    out, pos = [], 0
    for e in sorted(ces, key=lambda x: x.offset):
        start, end = e.offset * 2, (e.offset + e.length) * 2
        if start < pos or end > len(raw):
            continue
        out.append(raw[pos:start].decode("utf-16-le", "ignore"))
        char = raw[start:end].decode("utf-16-le", "ignore")
        out.append("{ce:%s:%s}" % (e.custom_emoji_id, char))
        pos = end
    out.append(raw[pos:].decode("utf-16-le", "ignore"))
    return "".join(out)


def message_button_text(message):
    """Text of an owner message with Premium emoji kept as markers."""
    return encode_custom_emoji_markers(
        getattr(message, "text", None) or "",
        getattr(message, "entities", None) or [],
    )


def count_custom_emoji(entities):
    return sum(1 for e in (entities or []) if getattr(e, "type", None) == "custom_emoji")


def premium_note(count):
    return f"\n💎 {count} Premium emoji save हुए." if count else ""
...
# USER PAGES
# ============================================================
def send_force_join_page(chat_id, user, warn=None):
    unjoined = get_unjoined_channels(user.id)
    txt,entities = _get_formatted_payload(get_set, 'fj_text', user)
    m_id = get_set('fj_media_id'); m_type = get_set('fj_media_type')
    verify_txt = get_set('fj_verify_btn')
    markup = _new_markup(row_width=2)
    btns=[]
    for ch in unjoined:
        if ch[2] and ch[3]: btns.append(make_url_button(ch[2],ch[3]))
    if btns: _markup_add(markup,*btns)
    _markup_add(markup, make_callback_button(verify_txt,'verify_force_join',default_style='primary',empty_text='Verify'))
    if m_type=='photo' and m_id!='NONE':
        return _send_media_formatted(bot,'photo',chat_id,m_id,txt,entities,markup,warn=warn)
    elif m_type=='video' and m_id!='NONE':
        return _send_media_formatted(bot,'video',chat_id,m_id,txt,entities,markup,warn=warn)
    else:
        return _send_formatted(bot,chat_id,txt,entities,markup,warn=warn)

def send_start_page(chat_id, user, warn=None):
    if warn is None:
        w = []
        try:
            return send_start_page(chat_id, user, warn=w)
        finally:
            _maybe_notify_owner('controller', ADMIN_ID, w)
    txt,entities = _get_formatted_payload(get_set, 'start_text', user)
    m_id=get_set('start_media_id'); m_type=get_set('start_media_type')
    markup=build_keyboard_from_string(get_set('start_buttons'))
    if m_type=='photo' and m_id!='NONE':
        return _send_media_formatted(bot,'photo',chat_id,m_id,txt,entities,markup,warn=warn)
...
def _message_html(message, caption=False):
    """Legacy HTML representation; kept as a fallback for old settings."""
    if caption:
...
def _entity_dict(entity):
    """Serialize Telegram MessageEntity without losing custom emoji IDs."""
    keys=('type','offset','length','url','language','custom_emoji_id')
    out={}
    for k in keys:
        v=getattr(entity,k,None)
        if v is not None:
            out[k]=v
    # text_mention/user is uncommon but preserving it is useful.
...
def _entity_objects(raw):
    """Turn stored JSON entities back into pyTelegramBotAPI objects.

    Never silently downgrade a custom-emoji entity to a normal Unicode emoji.
    If the installed pyTelegramBotAPI is too old to construct it, fail clearly.
    """
    ...
    for d in data or []:
        if not isinstance(d,dict) or not d.get('type'):
            continue
        kw={k:v for k,v in d.items() if k in ('type','offset','length','url','language','custom_emoji_id')}
        try:
            result.append(types.MessageEntity(**kw))
        except Exception as e:
            if kw.get('type') == 'custom_emoji':
                raise RuntimeError(
                    'Installed pyTelegramBotAPI cannot create Telegram custom_emoji MessageEntity. '
                    'Upgrade pyTelegramBotAPI before using Premium/custom emoji formatting.'
                ) from e
            raise
    return result
...
def _replace_template_with_entities(text, entities, user=None):
    """Replace placeholders while shifting Telegram UTF-16 entity offsets.

    This is preferable to converting through HTML because custom-emoji entities
    contain an emoji document ID and Telegram can render them exactly as sent.
    """
    text=str(text or '')
    if user is None:
        class _U: pass
    ...
    replacements={
        '{first_name}':str(getattr(user,'first_name','') or 'User'),
        '{username}':('@'+str(getattr(user,'username','')) if getattr(user,'username','') else ''),
        '{user_id}':str(getattr(user,'id','') or ''),
    }
    # Build output plus mapping from every original UTF-16 boundary to output.
    out=[]
    boundary={0:0}
    pos=0
    out_u16=0
    while pos < len(text):
        ...
        for token,repl in replacements.items():
            if text.startswith(token,pos):
                matched=(token,repl); break
        if matched:
            token,repl=matched
            token_u16=_utf16_len(token); repl_u16=_utf16_len(repl)
            out.append(repl)
            # Map the whole placeholder to the replacement span.
            boundary[_utf16_len(text[:pos])] = out_u16
            boundary[_utf16_len(text[:pos])+token_u16] = out_u16+repl_u16
            out_u16 += repl_u16
            pos += len(token)
        else:
            ch=text[pos]
            ch_u16=_utf16_len(ch)
            out.append(ch)
            boundary[_utf16_len(text[:pos])] = out_u16
            out_u16 += ch_u16
            boundary[_utf16_len(text[:pos+1])] = out_u16
            pos += 1
    original_total=_utf16_len(text)
    boundary[original_total]=out_u16

    def mapped(offset):
        if offset in boundary: return boundary[offset]
        # Offsets normally land on UTF-16 boundaries. For safety, clamp to the
        # nearest known boundary instead of silently dropping an entity.
        keys=sorted(boundary)
        prev=0
        for k in keys:
            if k>offset: break
            prev=k
        return boundary[prev]

    new_entities=[]
    for ent in entities or []:
        try:
            d=_entity_dict(ent) if not isinstance(ent,dict) else dict(ent)
            a=int(d.get('offset',0)); b=a+int(d.get('length',0))
            na=mapped(a); nb=mapped(b)
            if nb<=na: continue
            d['offset']=na; d['length']=nb-na
            new_entities.append(d)
        except Exception:
            pass
    return ''.join(out), _entity_objects(new_entities)

def _safe_placeholder(value):
    return html.escape(str(value or 'User'), quote=False)

def _render_template(value, user=None):
    """Legacy HTML template replacement for old stored HTML settings."""
    if value is None:
        return ''
    if user is None:
        class _U: pass
        user=_U(); user.first_name='User'; user.username=''; user.id=''
    replacements={
        '{first_name}': _safe_placeholder(getattr(user,'first_name','') or 'User'),
        '{username}': _safe_placeholder('@'+getattr(user,'username','') if getattr(user,'username','') else ''),
        '{user_id}': html.escape(str(getattr(user,'id','') or ''),quote=False),
    }
    out=str(value)
...
def _store_formatted_setting(setter, text_key, message):
    """Store raw text + Telegram entities + HTML fallback.

    Raw text and MessageEntity data are the canonical representation. This
    preserves bold/italic/quotes/spoilers/links/code AND custom emojis.
    """
    raw=getattr(message,'text',None)
    is_caption=False
    if raw is None:
        raw=getattr(message,'caption',None)
        is_caption=True
    raw=raw or ''
    setter(text_key, raw)
    entities=getattr(message,'caption_entities',None) if is_caption else getattr(message,'entities',None)
    if entities:
        setter(text_key+'_entities',json.dumps([_entity_dict(e) for e in entities],ensure_ascii=False))
    else:
        setter(text_key+'_entities','[]')
    rendered=_message_html(message,caption=is_caption)
    if rendered: setter(text_key+'_html',rendered)
    return count_custom_emoji(entities)

def _get_formatted_payload(getter,text_key,user=None):
    """Return (text, entities) for exact Telegram rendering."""
    raw=getter(text_key)
    ent_raw=getter(text_key+'_entities')
    if raw and raw!='NONE' and ent_raw and ent_raw not in ('NONE','[]'):
        try:
            data=json.loads(ent_raw)
            text, entities=_replace_template_with_entities(raw,data,user)
            return text,entities
        except Exception:
            pass
    # Backward compatibility for settings saved by older versions.
    html_value=getter(text_key+'_html')
    if html_value and html_value!='NONE':
        return _render_template(html_value,user),[]
    return _render_template(raw,user),[]

# ------------------------------------------------------------
# PREMIUM EMOJI SAFE SENDING
# ------------------------------------------------------------
# Telegram rule (Bot API 9.4+): custom/Premium emoji can be sent by a bot in
# private/group chats when the OWNER of the bot has Telegram Premium (or the
# bot owns a Fragment username).  The bot sends the exact entities that the
# owner typed, so the emoji arrives to every user exactly as the owner wrote it.
def _has_custom_emoji(entities):
    return any(getattr(e, 'type', None) == 'custom_emoji' for e in (entities or []))

def _without_custom_emoji(entities):
    return [e for e in (entities or []) if getattr(e, 'type', None) != 'custom_emoji']

_NOT_RETRYABLE = ('chat not found', 'peer_id_invalid', 'user is deactivated', 'blocked',
                  'forbidden', 'user not found', 'bot was kicked', 'not enough rights')

def _retryable(err):
    if getattr(err, 'error_code', None) != 400:
        return False
    low = str(err).lower()
    return not any(s in low for s in _NOT_RETRYABLE)

def _build_attempts(entities, reply_markup, plain=False):
    """Ordered send attempts: exact Premium formatting first, softer fallbacks after."""
    ents = list(entities or [])
    twin = getattr(reply_markup, '_twin', None) if getattr(reply_markup, '_has_icons', False) else None
    entity_mode = bool(ents) or plain
    pm = '' if entity_mode else None        # '' = no parse_mode, entities are used as-is
    attempts = [('full', ents, reply_markup, pm)]
    if twin is not None:
        attempts.append(('button_icons', ents, twin, pm))
    markup = twin if twin is not None else reply_markup
    if _has_custom_emoji(ents):
        attempts.append(('custom_emoji', _without_custom_emoji(ents), markup, ''))
    if ents:
        attempts.append(('entities', [], markup, ''))
    elif not entity_mode:
        attempts.append(('html', [], markup, ''))
    return attempts

_DROPPED_NOTE = 'SILENT_DROP: Telegram ne Premium emoji bina error ke hata diya.'

_FALLBACK_TEXT = {
    'button_icons': 'Button ka Premium emoji icon Telegram ne reject kiya - normal emoji dikhaya gaya.',
    'custom_emoji': 'Text ka Premium emoji Telegram ne reject kiya - normal emoji ke saath bheja gaya.',
    'entities': 'Formatting reject hui - plain text bheja gaya.',
    'html': 'HTML formatting reject hui - plain text bheja gaya.',
}

def _run_attempts(send_fn, attempts, warn=None):
    first_err = None
    for i, (label, ents, markup, pm) in enumerate(attempts):
        try:
            msg = send_fn(ents, markup, pm)
        except Exception as e:
            ...
            continue
        if i > 0:
            note = _FALLBACK_TEXT.get(label, label) + ' | ' + str(first_err)[:200]
            print('Premium-emoji fallback used:', note)
            if warn is not None:
                warn.append(note)
        elif _has_custom_emoji(ents):
            # Telegram may accept the request but silently strip custom emoji when the
            # bot is not allowed to use them. The returned message shows the truth.
            got = getattr(msg, 'entities', None) or getattr(msg, 'caption_entities', None) or []
            if not _has_custom_emoji(got):
                print('Premium emoji SILENTLY dropped by Telegram for this bot.')
                if warn is not None:
                    warn.append(_DROPPED_NOTE)
        return msg

def _send_formatted(bot_obj, chat_id, text, entities=None, reply_markup=None,
                    warn=None, plain=False, **kwargs):
    """Send text with the owner's exact Telegram entities (bold, quote, Premium emoji...).

    plain=True sends the text literally (no HTML parsing) - used for replies.
    """
    def _send(ents, markup, pm):
        kw = dict(kwargs)
        if pm is not None:
            kw['parse_mode'] = pm
        return bot_obj.send_message(chat_id, text, entities=ents or None,
                                    reply_markup=markup, **kw)
    return _run_attempts(_send, _build_attempts(entities, reply_markup, plain), warn)

def _send_media_formatted(bot_obj, kind, chat_id, media_id, caption, entities=None,
                          reply_markup=None, warn=None, plain=False, **kwargs):
    method = bot_obj.send_photo if kind == 'photo' else bot_obj.send_video

    def _send(ents, markup, pm):
        kw = dict(kwargs)
        if pm is not None:
            kw['parse_mode'] = pm
        return method(chat_id, media_id, caption=caption,
                      caption_entities=(ents or None), reply_markup=markup, **kw)
    return _run_attempts(_send, _build_attempts(entities, reply_markup, plain), warn)

def _notify_emoji_warning(chat_id, warn, bot_obj=None):
    """Tell the owner when Telegram refused / silently removed Premium emoji."""
    if not warn:
        return
    try:
        if any(str(w).startswith('SILENT_DROP') for w in warn):
            text = (
                "❌ <b>Telegram is bot se Premium emoji hata raha hai</b> (koi error nahi deta, "
                "chupchaap normal emoji bana deta hai).\n\n"
                "Bot ka code sahi hai - problem <b>bot ki eligibility</b> ki hai. Telegram rule: "
                "custom emoji sirf wahi bot bhej sakta hai jiske <b>owner account</b> me "
                "Telegram Premium ho (ya bot ke paas Fragment username ho).\n\n"
                "✅ Check karein:\n"
                "1) Premium wale account se @BotFather → /mybots me ye bot dikhna chahiye "
                "(bot usi account se bana ho).\n"
                "2) Agar bot kisi aur account se bana hai, to us account me Premium chahiye.\n"
                "3) Controller bot aur har connected bot ka owner alag check hota hai."
            )
        else:
            text = (
                "⚠️ <b>Premium emoji normal emoji ki tarah gaya.</b>\n\n"
                f"<code>{html.escape(str(warn[0])[:350], quote=False)}</code>\n\n"
                "✅ Fix: bot ke <b>owner account</b> (jisne @BotFather se bot banaya) me "
                "<b>Telegram Premium</b> active hona chahiye (ya bot ke paas Fragment username ho)."
            )
        (bot_obj or bot).send_message(chat_id, text, parse_mode='HTML')
    except Exception as e:
        print('emoji warning send failed:', e)

_EMOJI_NOTIFIED = {}

def _maybe_notify_owner(key, chat_id, warn):
    """Real-user sends: warn the owner at most once per hour per bot."""
    if not warn:
        return
    now = time.time()
    if now - _EMOJI_NOTIFIED.get(key, 0) < 3600:
        return
    _EMOJI_NOTIFIED[key] = now
    _notify_emoji_warning(chat_id, warn)

def _probe_custom_emoji(bot_obj, chat_id, emoji_id, char, length):
    """Send ONE Premium emoji, check whether Telegram kept it, delete it.
    Returns (True|False|None, error_text)."""
    try:
        ent = types.MessageEntity('custom_emoji', 0, length, custom_emoji_id=emoji_id)
        sent = bot_obj.send_message(chat_id, char, entities=[ent], parse_mode='')
    except Exception as e:
        return None, str(e)
    kept = _has_custom_emoji(getattr(sent, 'entities', None))
    try:
        bot_obj.delete_message(chat_id, sent.message_id)
    except Exception:
        pass
    return kept, ''

def _probe_and_report(sender_bot, target_chat, message, report_chat, label):
    """After the owner saves Premium emoji, test right away whether this bot can send them."""
    try:
        all_ce = [e for e in (getattr(message, 'entities', None) or [])
                  if getattr(e, 'type', None) == 'custom_emoji']
        ent = next((e for e in all_ce if getattr(e, 'custom_emoji_id', None)), None)
        if all_ce and ent is None:
            bot.send_message(report_chat,
                '❌ Premium emoji ki ID library se nahi mil rahi - pyTelegramBotAPI purani hai.\n'
                'requirements.txt me <code>pyTelegramBotAPI</code> ko latest karein '
                '(<code>pip install -U pyTelegramBotAPI</code>) aur redeploy karein.', parse_mode='HTML')
            return
        if ent is None or sender_bot is None:
            return
        raw = (message.text or '').encode('utf-16-le')
        char = raw[ent.offset * 2:(ent.offset + ent.length) * 2].decode('utf-16-le', 'ignore') or '🙂'
        ok, err = _probe_custom_emoji(sender_bot, target_chat, ent.custom_emoji_id, char, ent.length)
        if ok is True:
            bot.send_message(report_chat, f'✅ Test pass: {label} Premium emoji bhej sakta hai.', parse_mode='')
        elif ok is False:
            _notify_emoji_warning(report_chat, [_DROPPED_NOTE])
        else:
            bot.send_message(report_chat,
                f'ℹ️ Premium-emoji test nahi ho saka ({label}): {err[:150]}\n'
                'Connected bot ko ek baar /start karke text dobara save karein.', parse_mode='')
    except Exception as e:
        print('premium probe failed:', e)

def _see_formatted_text(bot_obj, chat_id, title, getter, key, user=None):
    """Show a saved text exactly as users will see it (Premium emoji included)."""
    warn = []
    try:
        txt, entities = _get_formatted_payload(getter, key, user)
        bot_obj.send_message(chat_id, title, parse_mode='HTML')
        _send_formatted(bot_obj, chat_id, txt, entities, warn=warn)
        _notify_emoji_warning(chat_id, warn, bot_obj)
    except Exception as e:
        bot_obj.send_message(chat_id, '❌ Text show nahi ho saka: ' + str(e)[:300], parse_mode='')

def _relay_reply(bot_obj, to_chat, message, warn=None):
    """Deliver an owner reply. Text keeps every Telegram entity (Premium emoji included)."""
    if getattr(message, 'content_type', '') == 'text' and message.text:
        return _send_formatted(bot_obj, to_chat, message.text, list(message.entities or []),
                               warn=warn, plain=True)
    return bot_obj.copy_message(to_chat, message.chat.id, message.message_id)

def _check_custom_emoji_support():
    try:
        e = types.MessageEntity(type='custom_emoji', offset=0, length=2, custom_emoji_id='1')
        if e.to_dict().get('custom_emoji_id') != '1':
            raise RuntimeError('custom_emoji_id not serialised')
        print('Premium/custom emoji support: OK')
    except Exception as ex:
        print('WARNING: installed pyTelegramBotAPI cannot handle custom emoji:', ex,
...
def managed_leave_channels(bot_db_id):
    """Channels whose leave events trigger DMOnLeave (separate from Force-Join channels)."""
    return mdb("SELECT id,channel_id,title,link FROM managed_leave_channels WHERE bot_id=%s ORDER BY id",(bot_db_id,),all_rows=True) or []
...
def managed_send_payload(mbot, chat_id, bot_db_id, prefix, pin=False, user=None, warn=None):
    if warn is None:
        w = []
        try:
            return managed_send_payload(mbot, chat_id, bot_db_id, prefix, pin, user, warn=w)
        finally:
            _maybe_notify_owner('m%s' % bot_db_id, OWNER_ID_FOR_BOT(bot_db_id), w)
    if user is None:
        user=managed_user_for_chat(bot_db_id,chat_id)
    txt,entities=_get_formatted_payload(lambda key: mget(bot_db_id,key),prefix+'_text',user)
    ...
    if mt=='photo' and mid!='NONE':
        msg=_send_media_formatted(mbot,'photo',chat_id,mid,txt,entities,markup,warn=warn)
    elif mt=='video' and mid!='NONE':
        msg=_send_media_formatted(mbot,'video',chat_id,mid,txt,entities,markup,warn=warn)
    else:
        msg=_send_formatted(mbot,chat_id,txt,entities,markup,warn=warn)
    if pin and msg:
...
def managed_start_page(mbot,bot_db_id,user,warn=None):
    if managed_blocked(bot_db_id,user.id): return
    missing=managed_unjoined(bot_db_id,user.id,mbot)
    if missing:
        mk=_new_markup(row_width=2)
        for ch in missing:
            if ch[2] and ch[3]: _markup_add(mk,make_url_button(ch[2],ch[3]))
        _markup_add(mk,make_callback_button(mget(bot_db_id,'fj_verify_btn'),f'mverify_{bot_db_id}',default_style='primary',empty_text='Verify'))
        txt,entities=_get_formatted_payload(lambda key: mget(bot_db_id,key),'fj_text',user)
        mid=mget(bot_db_id,'fj_media_id'); mt=mget(bot_db_id,'fj_media_type')
        if mt=='photo' and mid!='NONE':
            _send_media_formatted(mbot,'photo',user.id,mid,txt,entities,mk,warn=warn)
        elif mt=='video' and mid!='NONE':
            _send_media_formatted(mbot,'video',user.id,mid,txt,entities,mk,warn=warn)
        else:
            _send_formatted(mbot,user.id,txt,entities,mk,warn=warn)
        return
    managed_send_payload(mbot,user.id,bot_db_id,'start',mget(bot_db_id,'start_pin')=='YES',user,warn=warn)
...
# Last join/leave event seen per connected bot (in memory) - shown by "Check DMOnLeave".
LEAVE_DIAG = {}
_LEAVE_NOTIFIED = {}

def _leave_diag(bot_db_id, **kw):
    d = LEAVE_DIAG.setdefault(bot_db_id, {})
    if 'chat' in kw:                # a brand-new event
        d.clear(); d['ts'] = time.time()
    d.update(kw)

def _channel_matches(stored, chat):
    """Stored channel id may be -100123..., 123..., or @username - match all of them."""
    raw = str(stored or '').strip()
    real = str(getattr(chat, 'id', ''))
    if not raw:
        return False
    if raw == real:
        return True
    uname = (getattr(chat, 'username', None) or '').lower()
    if uname and raw.lower().lstrip('@') == uname:
        return True
    return raw.lstrip('-') == real.replace('-100', '', 1).lstrip('-')

def _leave_channel_configured(bot_db_id, chat):
    """True if leaving this chat should trigger DMOnLeave.

    Uses the LeaveDM Channels list. Backward compatible: while that list is still
    empty, the old behaviour (Force-Join channels) keeps working.
    """
    rows = managed_leave_channels(bot_db_id)
    if not rows:
        rows = managed_channels(bot_db_id)
    return any(_channel_matches(r[1], chat) for r in rows)

def _leave_fail_reason(err):
    low = str(err).lower()
    if "can't initiate" in low or 'cant initiate' in low:
        return "is user ne connected bot ko /start nahi kiya (Telegram bot ko pehla message nahi bhejne deta)"
    if 'blocked' in low:
        return 'is user ne bot ko block kar rakha hai'
    if 'deactivated' in low:
        return 'is user ka Telegram account delete ho chuka hai'
    if 'chat not found' in low:
        return 'chat nahi mili (user ne kabhi bot start nahi kiya)'
    return 'Telegram error: ' + str(err)[:200]

def _notify_leave_problem(bot_db_id, user_id, reason):
    """Owner ko batao ki DMOnLeave kyon nahi gaya (20 minute me ek baar)."""
    now = time.time()
    if now - _LEAVE_NOTIFIED.get(bot_db_id, 0) < 1200:
        return
    _LEAVE_NOTIFIED[bot_db_id] = now
    try:
        bot.send_message(
            OWNER_ID_FOR_BOT(bot_db_id),
            '⚠️ <b>DMOnLeave nahi gaya</b>\n\n'
            f'User: <code>{user_id}</code>\n'
            f'Kaaran: {html.escape(reason, quote=False)}\n\n'
            '<i>Isi tarah ke aur failures ki khabar 20 minute tak nahi dunga. '
            'Poori jaanch: DMOnLeave settings → 🔎 Check DMOnLeave.</i>',
            parse_mode='HTML'
        )
    except Exception as e:
        print('leave notify failed:', e)
...
def managed_dm_on_leave(mbot,bot_db_id,user_id,user):
    ...
    if mget(bot_db_id,'dmleave_enabled') != 'YES':
        ...
        _leave_diag(bot_db_id, result='❌ DMOnLeave OFF hai - DMOnLeave settings me "Next ➡️" dabakar ON karein')
        _notify_leave_problem(bot_db_id, user_id, 'DMOnLeave OFF hai (DMOnLeave settings me "Next ➡️" dabayein)')
        return False
    if managed_blocked(bot_db_id,user_id):
        print(f'DMOnLeave blocked user {user_id} for bot {bot_db_id}')
        _leave_diag(bot_db_id, result='⏭️ Skip: ye user bot me "blocked" hai (pehle channel se kick/ban hua tha)')
        return False
    try:
        ...
        _leave_diag(bot_db_id, result='✅ DM bhej diya gaya')
        return bool(msg)
    except Exception as e:
        print(f'DMOnLeave send failed: bot={bot_db_id} user={user_id} error={e}')
        reason = _leave_fail_reason(e)
        _leave_diag(bot_db_id, result='❌ DM fail: ' + reason)
        _notify_leave_problem(bot_db_id, user_id, reason)
        return False

def _ago(ts):
    sec = int(time.time() - ts)
    if sec < 60: return f'{sec} sec pehle'
    if sec < 3600: return f'{sec // 60} min pehle'
    return f'{sec // 3600} ghante pehle'

def managed_leave_diag(chat_id, bid):
    """One-tap health check for DMOnLeave (no Railway logs needed)."""
    row = managed_info(bid)
    if not row:
        return
    mbot = MANAGED_BOTS.get(bid)
    enabled = mget(bid, 'dmleave_enabled') == 'YES'
    lines = [f'🔎 <b>DMOnLeave check</b> - @{html.escape(str(row[4] or row[5] or ""))}', '']
    lines.append('• DMOnLeave: ' + ('✅ ON' if enabled else '❌ OFF (neeche "ON karo" dabayein)'))
    lines.append('• Bot chal raha hai: ' + ('✅ haan' if mbot else '❌ nahi - Reconnect karein'))
    chans = managed_leave_channels(bid)
    if chans:
        lines.append('• 📺 LeaveDM Channels:')
    else:
        chans = managed_channels(bid)
        if chans:
            lines.append('• 📺 LeaveDM list khali hai, isliye abhi Force Join channels use ho rahe hain:')
        else:
            lines.append('• Channels: ❌ koi channel add nahi hai - 📺 LeaveDM Channels me add karein')
    for ch in chans:
        status = '?'
        try:
            me = mbot.get_me()
            status = mbot.get_chat_member(ch[1], me.id).status
        except Exception as e:
            status = 'error: ' + str(e)[:80]
        ok = status in ('administrator', 'creator')
        name = html.escape(parse_colors(ch[2]) or str(ch[1]), quote=False)
        lines.append(f'• {name} <code>{html.escape(str(ch[1]))}</code>: bot ' +
                     ('✅ admin hai' if ok else '❌ admin nahi (' + html.escape(str(status)) + ')'))
    d = LEAVE_DIAG.get(bid)
    if d:
        lines.append('')
        lines.append(f'📨 <b>Last event</b> ({_ago(d["ts"])})')
        lines.append(f'{html.escape(str(d.get("chat","")))} | user <code>{d.get("user")}</code> | '
                     f'{d.get("old")} → {d.get("new")}')
        lines.append(html.escape(str(d.get('result', '...')), quote=False))
    else:
        lines.append('')
        lines.append('ℹ️ Restart ke baad Telegram ne abhi tak koi join/leave event bheja hi nahi.\n'
                     'Test: kisi account se (jisne bot /start kiya ho) channel join karke leave karein, '
                     'phir ye check dobara dabayein. Agar event tab bhi na dikhe to Telegram event bhej hi '
                     'nahi raha - bot ko channel me admin banakar dobara add karein.')
    mk = types.InlineKeyboardMarkup(row_width=1)
    if not enabled:
        mk.add(types.InlineKeyboardButton('✅ DMOnLeave ON karo', callback_data=f'mleaveon_{bid}'))
    mk.add(types.InlineKeyboardButton('🔄 Dobara check', callback_data=f'mdiag_{bid}'),
           types.InlineKeyboardButton('⬅️ Back', callback_data=f'mbot_{bid}'))
...
def managed_register_handlers(mbot,bot_db_id):
    ...
    def _member_update(update):
        try:
            ...
            _leave_diag(bot_db_id, chat=getattr(chat,'title','') or cid, user=user.id, old=old, new=new,
                        result='event mila, process ho raha hai...')

            in_fj=any(_channel_matches(ch[1],chat) for ch in managed_channels(bot_db_id))
            in_leave=_leave_channel_configured(bot_db_id,chat)
            if not (in_fj or in_leave):
                _leave_diag(bot_db_id, result=f'⏭️ Ignore: channel {cid} na Force Join list me hai na 📺 LeaveDM Channels list me')
                return

            if new in ('member','administrator','creator') and old in ('left','kicked','restricted',''):
                if not in_fj:
                    _leave_diag(bot_db_id, result='ℹ️ Join event - ye LeaveDM channel hai (AutoDM sirf Force Join channels par chalta hai)')
                    return
                managed_add_user(bot_db_id,user)
                try:
                    mdb("UPDATE managed_users SET status='active' WHERE bot_id=%s AND user_id=%s AND status IN ('left','blocked')",
                        (bot_db_id,user.id),commit=True)
                except Exception:
                    pass
                _leave_diag(bot_db_id, result='➡️ Join event - AutoDM process hua')
                managed_autodm(mbot,bot_db_id,user.id,user)
                return

            if new == 'left' and old in ('member','administrator','creator','restricted'):
                if not in_leave:
                    _leave_diag(bot_db_id, result='⏭️ Ye Force Join channel hai, 📺 LeaveDM Channels list me nahi - DM nahi gaya')
                    return
                managed_dm_on_leave(mbot,bot_db_id,user.id,user)
                return

            if new == 'kicked':
                _leave_diag(bot_db_id, result='⏭️ User kick/ban hua - DMOnLeave sirf "left" par jaata hai')
                try:
            ...
            _leave_diag(bot_db_id, result=f'⏭️ Ignore: status change {old} → {new}')
        except Exception as e:
    ...
    def _member_service(message):
        try:
            ...
            if not _leave_channel_configured(bot_db_id,message.chat): return
            user=getattr(message,'left_chat_member',None)
            if not user: return
            print(f'DMOnLeave service event: bot={bot_db_id} chat={cid} user={user.id}')
            _leave_diag(bot_db_id, chat=getattr(message.chat,'title','') or cid, user=user.id, old='member', new='left(service msg)', result='service message mila')
            managed_dm_on_leave(mbot,bot_db_id,user.id,user)
        except Exception as e:
...
def managed_home(chat_id,bot_db_id):
    ...
    for title,data in [
        ...
        ('📺 LeaveDM Channels',f'ms_{bot_db_id}_lch'),
...
def managed_settings_keyboard(bot_db_id,prefix,next_cb):
    ...
    if prefix=='dmleave':
        mk.add(types.InlineKeyboardButton('📺 LeaveDM Channels',callback_data=f'ms_{bot_db_id}_lch'))
        mk.add(types.InlineKeyboardButton('🔎 Check DMOnLeave',callback_data=f'mdiag_{bot_db_id}'))
...
def managed_save_input(message,bot_db_id,prefix,kind):
    ...
    if kind=='text':
        ...
        saved_ce=_store_formatted_setting(lambda k,v: mset(bot_db_id,k,v), prefix+'_text', message)
    elif kind=='buttons':
        if not message.text: bot.send_message(message.chat.id,'❌ Buttons text भेजें.'); return
        mset(bot_db_id,prefix+'_buttons',message_button_text(message))
        saved_ce=count_custom_emoji(message.entities)
    elif kind=='media':
    ...
    bot.send_message(message.chat.id,'✅ Setting saved.'+premium_note(saved_ce))
    if saved_ce:
        _probe_and_report(MANAGED_BOTS.get(bot_db_id),OWNER_ID_FOR_BOT(bot_db_id),message,message.chat.id,'ye connected bot')
...
def managed_preview(chat_id,bot_db_id,prefix):
    ...
    warn=[]
    try:
        # Preview uses the connected bot itself, so the real Telegram entity
        # rendering is exercised instead of a controller-side approximation.
        managed_send_payload(mbot,chat_id,bot_db_id,prefix,False,managed_user_for_chat(bot_db_id,chat_id),warn=warn)
        _notify_emoji_warning(chat_id,warn)
    except Exception as e:
        bot.send_message(chat_id,'❌ Preview failed: '+str(e)[:500],parse_mode='')
...
def managed_broadcast(bot_db_id,owner_chat_id):
    ...
    sent=failed=0
    warn=[]
    pin=mget(bot_db_id,'bcast_pin')=='YES'
    for (uid,) in rows:
        try: managed_send_payload(mbot,uid,bot_db_id,'bcast',pin,warn=warn); sent+=1
        except Exception: failed+=1
        time.sleep(0.03)
    bot.send_message(owner_chat_id,f'📢 <b>Broadcast finished</b>\n\n✅ Sent: {sent}\n❌ Failed: {failed}')
    _notify_emoji_warning(owner_chat_id,warn)

def managed_send_reply_text_connected(message,bot_db_id,user_id):
    """Send an owner reply through the connected bot with original Telegram entities (Premium emoji kept)."""
    mbot=MANAGED_BOTS.get(bot_db_id)
    ...
    try:
        ...
        _send_formatted(mbot,user_id,message.text,entities,warn=warn,plain=True)
        mbot.send_message(message.chat.id,'✅ Reply sent.')
        _notify_emoji_warning(message.chat.id,warn,mbot)
    except Exception as e:
        try: mbot.send_message(message.chat.id,'❌ Reply failed: '+str(e)[:300],parse_mode='')
        except Exception: pass
...
def managed_send_reply_text(message,bot_db_id,user_id):
    ...
    try:
        ...
        _notify_emoji_warning(message.chat.id,warn)
    except Exception as e:
        bot.send_message(message.chat.id,'❌ Reply failed: '+str(e)[:300],parse_mode='')
...
def managed_add_channel(message,bot_db_id):
    try:
        parts=[x.strip() for x in message_button_text(message).split('|')]
        if len(parts)!=3: raise ValueError()
        cid,title,link=parts
        mbot=MANAGED_BOTS.get(bot_db_id)
        if not mbot: raise ValueError('Bot is not running')
        chat_obj=mbot.get_chat(cid)
        me=mbot.get_me()
        ...
        # Always store the real numeric chat id (so leave/join events always match), and
        # drop an older @username / duplicate entry of the same channel.
        real_cid=str(chat_obj.id)
        if cid!=real_cid:
            mdb("DELETE FROM managed_channels WHERE bot_id=%s AND channel_id=%s",(bot_db_id,cid),commit=True)
        mdb("INSERT INTO managed_channels(bot_id,channel_id,title,link) VALUES(%s,%s,%s,%s) ON CONFLICT(bot_id,channel_id) DO UPDATE SET title=EXCLUDED.title,link=EXCLUDED.link",(bot_db_id,real_cid,title,link),commit=True)
...
def managed_channels_menu(chat_id,bot_db_id):
    ...
    for rid,cid,title,link in rows: mk.add(types.InlineKeyboardButton('🗑️ '+(parse_colors(title) or cid),callback_data=f'mchdel_{bot_db_id}_{rid}'))
...
def managed_leave_channels_menu(chat_id,bot_db_id):
    rows=managed_leave_channels(bot_db_id); mk=types.InlineKeyboardMarkup(row_width=1)
    lines=[]
    for rid,cid,title,link in rows:
        name=(title or cid)
        mk.add(types.InlineKeyboardButton('🗑️ '+name,callback_data=f'mlchdel_{bot_db_id}_{rid}'))
        lines.append(f'• {html.escape(str(name),quote=False)} <code>{html.escape(str(cid))}</code>')
    mk.add(types.InlineKeyboardButton('➕ Add LeaveDM Channel',callback_data=f'mlchadd_{bot_db_id}'))
    mk.add(types.InlineKeyboardButton('👋 DMOnLeave Settings',callback_data=f'ms_{bot_db_id}_leave'),
           types.InlineKeyboardButton('🔎 Check',callback_data=f'mdiag_{bot_db_id}'))
    mk.add(types.InlineKeyboardButton('⬅️ Back',callback_data=f'mbot_{bot_db_id}'))
    body=('\n'.join(lines) if lines else '<i>अभी कोई channel नहीं है.</i>')
    note=('' if rows else '\n\nℹ️ जब तक यहाँ channel add नहीं करते, पुराना तरीका (Force Join channels) चलता रहेगा.')
    bot.send_message(chat_id,
        '👋 <b>LeaveDM Channels</b>\n\n'
        'इन channels से कोई user <b>leave</b> करेगा तो उसे DMOnLeave वाला DM जाएगा.\n'
        'यह list <b>Force Join channels से बिल्कुल अलग</b> है.\n\n'
        +body+note+
        '\n\n⚠️ Connected bot इन channels का <b>admin</b> होना चाहिए और user ने bot /start किया होना चाहिए.',
        reply_markup=mk)

def managed_leave_channel_prompt(chat_id,bot_db_id):
    m=bot.send_message(chat_id,
        '➕ <b>LeaveDM Channel जोड़ें</b>\n\n'
        'Channel ID या @username भेजें (एक से ज़्यादा हों तो हर line में एक):\n\n'
        '<code>-1001234567890</code>\n<code>@mychannel</code>\n'
        '<code>-1001234567890 | Channel Name</code>\n\n'
        'Connected bot उस channel का admin होना चाहिए.')
    bot.register_next_step_handler(m,managed_add_leave_channel,bot_db_id)

def managed_add_leave_channel(message,bot_db_id):
    mbot=MANAGED_BOTS.get(bot_db_id)
    if not mbot:
        bot.send_message(message.chat.id,'❌ Connected bot is not running. Reconnect करें.'); return
    if not message.text:
        bot.send_message(message.chat.id,'❌ Channel ID या @username text में भेजें.'); return
    results=[]
    for line in message.text.splitlines():
        ...
        cid=parts[0]
        if cid.lower().startswith(('https://t.me/','http://t.me/','t.me/')):
            tail=cid.rstrip('/').split('/')[-1]
            if tail and not tail.startswith('+'): cid='@'+tail
        try:
            chat_obj=mbot.get_chat(cid)
            me=mbot.get_me()
            st=getattr(mbot.get_chat_member(chat_obj.id,me.id),'status','')
            if st not in ('administrator','creator'):
                raise ValueError('Connected bot इस channel में admin नहीं है')
            real_cid=str(chat_obj.id)
            title=(parts[1] if len(parts)>1 and parts[1] else '') or getattr(chat_obj,'title','') or real_cid
            uname=getattr(chat_obj,'username',None)
            link=(parts[2] if len(parts)>2 and parts[2] else '') or (f'https://t.me/{uname}' if uname else '')
            if cid!=real_cid:
                mdb("DELETE FROM managed_leave_channels WHERE bot_id=%s AND channel_id=%s",(bot_db_id,cid),commit=True)
            mdb("INSERT INTO managed_leave_channels(bot_id,channel_id,title,link) VALUES(%s,%s,%s,%s) "
                "ON CONFLICT(bot_id,channel_id) DO UPDATE SET title=EXCLUDED.title,link=EXCLUDED.link",
                (bot_db_id,real_cid,title,link),commit=True)
            results.append(f'✅ {html.escape(str(title),quote=False)} <code>{real_cid}</code>')
        except Exception as e:
            results.append(f'❌ <code>{html.escape(cid,quote=False)}</code> - {html.escape(str(e)[:150],quote=False)}')
    if not results:
        bot.send_message(message.chat.id,'❌ कोई channel नहीं मिला.'); return
    bot.send_message(message.chat.id,'\n'.join(results))
    managed_leave_channels_menu(message.chat.id,bot_db_id)
...
def handle_callbacks(call):
    ...
    if call.data.startswith('ms_'):
        ...
        elif action=='lch': managed_leave_channels_menu(chat_id,bid)
    ...
    if call.data.startswith('msee_'):
        ...
        if not managed_is_owner(bid,user_id): return
        if kind=='text':
            _see_formatted_text(bot,chat_id,f'👀 <b>{prefix} {kind}</b>',lambda key: mget(bid,key),prefix+'_text',managed_user_for_chat(bid,chat_id)); return
    ...
    if call.data.startswith('mreply_'):
        ...
        return
    if call.data.startswith('mlchadd_'):
        bid=int(call.data.split('_',1)[1])
        if not managed_is_owner(bid,user_id):
            bot.answer_callback_query(call.id,'❌ यह आपका connected bot नहीं है.',show_alert=True); return
        bot.answer_callback_query(call.id)
        managed_leave_channel_prompt(chat_id,bid); return
    if call.data.startswith('mlchdel_'):
        _,bid_s,rid_s=call.data.split('_',2); bid=int(bid_s); rid=int(rid_s)
        if managed_is_owner(bid,user_id):
            bot.answer_callback_query(call.id,'🗑️ Removed')
            mdb('DELETE FROM managed_leave_channels WHERE id=%s AND bot_id=%s',(rid,bid),commit=True)
            managed_leave_channels_menu(chat_id,bid)
        return
    if call.data.startswith('mdiag_'):
        bid=int(call.data.split('_',1)[1])
        if not managed_is_owner(bid,user_id):
            bot.answer_callback_query(call.id,'❌ यह आपका connected bot नहीं है.',show_alert=True); return
        bot.answer_callback_query(call.id,'Check ho raha hai...')
        managed_leave_diag(chat_id,bid); return
    if call.data.startswith('mleaveon_'):
        bid=int(call.data.split('_',1)[1])
        if not managed_is_owner(bid,user_id):
            bot.answer_callback_query(call.id,'❌ यह आपका connected bot नहीं है.',show_alert=True); return
        mset(bid,'dmleave_enabled','YES')
        bot.answer_callback_query(call.id,'✅ DMOnLeave ON')
        managed_leave_diag(chat_id,bid); return
    ...
    # --------------------------------------------------------
        ...
        _see_formatted_text(bot, chat_id, "📝 टेक्स्ट:", get_set, "start_text", get_main_user(chat_id))
        ...
        _w = []
        try:
            send_start_page(chat_id, call.from_user, warn=_w)
        except Exception as e:
            bot.send_message(chat_id, "❌ Preview failed: " + str(e)[:400], parse_mode="")
        _notify_emoji_warning(chat_id, _w)
    ...
    # --------------------------------------------------------
        ...
        _see_formatted_text(bot, chat_id, "📝 FJ Text:", get_set, "fj_text", get_main_user(chat_id))
        ...
        _w = []
        try:
            send_force_join_page(chat_id, call.from_user, warn=_w)
        except Exception as e:
            bot.send_message(chat_id, "❌ Preview failed: " + str(e)[:400], parse_mode="")
        _notify_emoji_warning(chat_id, _w)
    ...
    # --------------------------------------------------------
        ...
        _see_formatted_text(bot, chat_id, "📝 Broadcast Text:", get_set, "bcast_text", get_main_user(chat_id))
...
def save_start_text(message):
    ...
    n = _store_formatted_setting(set_val, "start_text", message)
    bot.send_message(
        ADMIN_ID,
        "✅ Start Text अपडेट!" + premium_note(n)
    )
    if n:
        _probe_and_report(bot, ADMIN_ID, message, ADMIN_ID, "controller bot")
...
def save_fj_text(message):
    ...
    n = _store_formatted_setting(set_val, "fj_text", message)
    bot.send_message(
        ADMIN_ID,
        "✅ Force Join Text अपडेट!" + premium_note(n)
    )
    if n:
        _probe_and_report(bot, ADMIN_ID, message, ADMIN_ID, "controller bot")
...
def save_fj_button(message):
    ...
    bot.send_message(
        ADMIN_ID,
        "✅ Verify Button अपडेट!" + premium_note(count_custom_emoji(message.entities))
    )
...
def save_start_buttons(message):
    ...
    bot.send_message(
        ADMIN_ID,
        "✅ Start Buttons अपडेट!" + premium_note(count_custom_emoji(message.entities))
    )
...
def save_bcast_text(message):
    ...
    n = _store_formatted_setting(set_val, "bcast_text", message)

    bot.send_message(
        ADMIN_ID,
        "✅ Broadcast Text अपडेट!" + premium_note(n)
    )
    if n:
        _probe_and_report(bot, ADMIN_ID, message, ADMIN_ID, "controller bot")
...
def save_bcast_buttons(message):
    ...
    bot.send_message(
        ADMIN_ID,
        "✅ Broadcast Buttons अपडेट!" + premium_note(count_custom_emoji(message.entities))
    )
...
def process_add_ch(message):
    try:
        ...
        parts = [
            x.strip()
            for x in message_button_text(message).split("|")
        ]
...
def send_broadcast_preview(chat_id):
    txt,entities=_get_formatted_payload(get_set,'bcast_text',get_main_user(chat_id))
    m_id=get_set('bcast_media_id'); m_type=get_set('bcast_media_type')
    markup=build_keyboard_from_string(get_set('bcast_buttons'))
    warn=[]
    try:
        if m_type=='photo' and m_id!='NONE':
            msg=_send_media_formatted(bot,'photo',chat_id,m_id,txt,entities,markup,warn=warn)
        elif m_type=='video' and m_id!='NONE':
            msg=_send_media_formatted(bot,'video',chat_id,m_id,txt,entities,markup,warn=warn)
        else:
            msg=_send_formatted(bot,chat_id,txt,entities,markup,warn=warn)
        _notify_emoji_warning(chat_id,warn)
        return msg
    except Exception as e:
        bot.send_message(chat_id,'❌ Preview error: '+str(e)[:500],parse_mode='')

def execute_broadcast(admin_chat_id):
    rows = db_execute("SELECT user_id FROM users", fetchall=True) or []
    ...
    for u in users:
        try:
            txt,entities=_get_formatted_payload(get_set,'bcast_text',get_main_user(u))
            if m_type == "photo" and m_id != "NONE":
                m = _send_media_formatted(bot,'photo',u,m_id,txt,entities,markup,warn=warn)
            elif m_type == "video" and m_id != "NONE":
                m = _send_media_formatted(bot,'video',u,m_id,txt,entities,markup,warn=warn)
            else:
                m = _send_formatted(bot,u,txt,entities,markup,warn=warn)
            ...
            try:
                ...
                print("Broadcast status update warning:", db_err)
            ...
            # Only a real "user blocked / chat gone" error marks a user as blocked.
            low = str(e).lower()
    ...
    _notify_emoji_warning(admin_chat_id, warn)
...
def admin_reply_to_user(message):
    ...
    try:
        warn = []
        _relay_reply(bot, user_id, message, warn=warn)

        bot.send_message(
            ADMIN_ID,
            "✅ Reply user को भेज दिया गया।"
        )
        _notify_emoji_warning(ADMIN_ID, warn)
