...
def managed_entity_json(entities):
    """Serialize Telegram message entities so Controller formatting survives storage."""
    out=[]
    for e in (entities or []):
        if hasattr(e, 'to_dict'):
            ...
            d={}
            for k in ('type','offset','length','url','language','custom_emoji_id'):
                v=getattr(e,k,None)
                if v is not None: d[k]=v
            u=getattr(e,'user',None)
            if u is not None:
                d['user'] = u.to_dict() if hasattr(u,'to_dict') else getattr(u,'id',None)
        out.append({k:v for k,v in d.items() if v is not None})
    return json.dumps(out, ensure_ascii=False)

def managed_entity_objects(raw):
    """Rebuild pyTelegramBotAPI MessageEntity objects from stored JSON."""
    ...
    try:
        ...
        for d in arr or []:
            ...
            kw={k:d[k] for k in ('url','language','custom_emoji_id') if k in d}
            # text_mention entities may contain a serialized user object.
            if d.get('user') and isinstance(d['user'],dict):
                try: kw['user']=types.User.de_json(json.dumps(d['user']))
                except Exception: pass
            result.append(types.MessageEntity(d.get('type',''), int(d.get('offset',0)), int(d.get('length',0)), **kw))
        return result or None
    except Exception as e:
        print('managed entity decode:',e)
        return None

def _u16len(value):
    return len((value or '').encode('utf-16-le')) // 2

def managed_template_render(bot_db_id, prefix, chat_id=None):
    """Render placeholders and remap stored Telegram entity offsets after replacement."""
    original=mget(bot_db_id,prefix+'_text') or ''
    raw_entities=mget(bot_db_id,prefix+'_entities')
    entities=managed_entity_objects(raw_entities)
    first_name='User'; username=''; user_id=str(chat_id or '')
    row=mdb('SELECT first_name,username,user_id FROM managed_users WHERE bot_id=%s AND user_id=%s',(bot_db_id,chat_id),one=True) if chat_id else None
    if row:
        first_name=row[0] or 'User'; username=row[1] or ''; user_id=str(row[2])
    replacements={
        '{first_name}': first_name,
        '{username}': ('@'+username if username else ''),
        '{user_id}': user_id,
    }
    # Build the final string and a UTF-16 boundary map from original offsets to new offsets.
    out=[]; mapping={0:0}; pos=0; newpos=0
    while pos < len(original):
        found=None
        for token,repl in replacements.items():
            if original.startswith(token,pos):
                found=(token,repl); break
        if found:
            token,repl=found
            token_u=_u16len(token); repl_u=_u16len(repl)
            out.append(repl)
            newpos += repl_u
            pos += len(token)
            mapping[_u16len(original[:pos])]=newpos
        else:
            ch=original[pos]
            out.append(ch)
            newpos += _u16len(ch)
    ...
    if not entities:
        return final, None
    # Map entity boundaries. If an entity spans a placeholder, its length expands/contracts too.
    fixed=[]
    for e in entities:
        old_start=int(e.offset); old_end=old_start+int(e.length)
        starts=[k for k in mapping if k<=old_start]
        ends=[k for k in mapping if k<=old_end]
        new_start=mapping[max(starts)] if starts else old_start
        new_end=mapping[max(ends)] if ends else old_end
        e.offset=new_start; e.length=max(0,new_end-new_start)
        fixed.append(e)
    return final, fixed

def managed_template_text(bot_db_id, prefix, chat_id=None):
    return managed_template_render(bot_db_id,prefix,chat_id)[0]

def managed_send_payload(mbot, chat_id, bot_db_id, prefix, pin=False):
    txt,entities=managed_template_render(bot_db_id,prefix,chat_id)
    ...
    try:
        if mt=='photo' and mid!='NONE':
            msg=mbot.send_photo(chat_id,mid,caption=txt,caption_entities=entities,reply_markup=markup)
        elif mt=='video' and mid!='NONE':
            msg=mbot.send_video(chat_id,mid,caption=txt,caption_entities=entities,reply_markup=markup)
        else:
            msg=mbot.send_message(chat_id,txt,entities=entities,reply_markup=markup)
    except Exception as e:
        # Keep delivery compatible with older pyTelegramBotAPI versions.
        try:
...
def managed_dm_on_leave(mbot,bot_db_id,user_id,user):
    """Send configured DMOnLeave payload after a channel leave event."""
    if mget(bot_db_id,'dmleave_enabled') != 'YES' or managed_blocked(bot_db_id,user_id):
        return False
    try:
        # Ensure the latest user identity is available for {first_name} etc.
        managed_add_user(bot_db_id,user)
        return bool(managed_send_payload(mbot,user_id,bot_db_id,'dmleave',mget(bot_db_id,'dmleave_pin')=='YES'))
    except Exception as e:
...
def managed_register_handlers(mbot,bot_db_id):
    ...
    @mbot.callback_query_handler(func=lambda call: call.data.startswith(f'mreply_{bot_db_id}_'))
    def _managed_reply_callback(call):
        try:
            _,bid_s,uid_s=call.data.split('_',2)
            bid=int(bid_s); uid=int(uid_s)
            if bid != bot_db_id or call.from_user.id != OWNER_ID_FOR_BOT(bot_db_id):
                mbot.answer_callback_query(call.id,'❌ Owner only',show_alert=True); return
            mbot.answer_callback_query(call.id)
            prompt=mbot.send_message(call.from_user.id,'↩️ Reply भेजें. Formatting, quote, bold, italic, links, spoiler आदि preserved रहेंगे.')
            # This next-step runs on the connected bot, so the reply reaches the
            # actual user through the same connected bot instance.
            mbot.register_next_step_handler(prompt, lambda msg: managed_send_reply_from_connected_bot(msg,bot_db_id,uid))
        except Exception as e:
    ...
    def _live(message):
        ...
        try:
            ...
            # Confirm successful delivery to the connected bot owner as well.
            try:
                owner=OWNER_ID_FOR_BOT(bot_db_id)
                mbot.send_message(owner,
                    '✉️ <b>MESSAGE SENT SUCCESSFULLY</b>\n\n'
                    f'👤 <b>{message.from_user.first_name or "User"}</b>\n'
...
def managed_save_input(message,bot_db_id,prefix,kind):
    ...
    if kind=='text':
        ...
        mset(bot_db_id,prefix+'_entities',managed_entity_json(message.entities))
    elif kind=='buttons':
...
def managed_send_reply_from_connected_bot(message,bot_db_id,user_id):
    """Send owner reply through the connected bot while preserving Telegram entities."""
    mbot=MANAGED_BOTS.get(bot_db_id)
    if not mbot:
        return
    try:
        ...
        if message.text:
            ...
            mbot.send_message(user_id,message.text,entities=entities)
        elif message.photo:
            cap=message.caption or ''
            mbot.send_photo(user_id,message.photo[-1].file_id,caption=cap,caption_entities=message.caption_entities)
        elif message.video:
            cap=message.caption or ''
            mbot.send_video(user_id,message.video.file_id,caption=cap,caption_entities=message.caption_entities)
        elif message.document:
            mbot.send_document(user_id,message.document.file_id,caption=message.caption or '',caption_entities=message.caption_entities)
        elif message.sticker:
            mbot.send_sticker(user_id,message.sticker.file_id)
        elif message.voice:
            mbot.send_voice(user_id,message.voice.file_id,caption=message.caption or '',caption_entities=message.caption_entities)
        elif message.audio:
            mbot.send_audio(user_id,message.audio.file_id,caption=message.caption or '',caption_entities=message.caption_entities)
        else:
            mbot.copy_message(user_id,message.chat.id,message.message_id)
        mbot.send_message(message.chat.id,'✅ Reply sent successfully.')
    except Exception as e:
        mbot.send_message(message.chat.id,'❌ Reply failed: '+str(e)[:300])

def managed_send_reply_text(message,bot_db_id,user_id):
    """Legacy controller-bot reply path; preserve all Telegram text entities."""
    mbot=MANAGED_BOTS.get(bot_db_id)
    if not mbot:
        bot.send_message(message.chat.id,'❌ Connected bot unavailable.')
        return
    try:
        if message.text:
            mbot.send_message(user_id,message.text,entities=message.entities)
        elif message.photo:
            mbot.send_photo(user_id,message.photo[-1].file_id,caption=message.caption or '',caption_entities=message.caption_entities)
        elif message.video:
            mbot.send_video(user_id,message.video.file_id,caption=message.caption or '',caption_entities=message.caption_entities)
        elif message.document:
            mbot.send_document(user_id,message.document.file_id,caption=message.caption or '',caption_entities=message.caption_entities)
        else:
            mbot.copy_message(user_id,message.chat.id,message.message_id)
...
def handle_callbacks(call):
    ...
    # Everything else below is admin-only for the controller's original bot.
    if not is_admin(user_id):
...
def save_start_text(message):
    ...
    set_val("start_entities", managed_entity_json(message.entities))
    bot.send_message(
        ADMIN_ID,
        "✅ Start Text अपडेट!"
    )
...
def save_bcast_text(message):
    ...
    set_val("bcast_entities", managed_entity_json(message.entities))
